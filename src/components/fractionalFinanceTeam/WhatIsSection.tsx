...
const SupportCard = ({
  ...
  area: (typeof supportAreas)[number];
  index: number;
}) => {
  ...
  return (
    <motion.div
      ...
      className="group h-full p-6 bg-slate-50 rounded-2xl border border-slate-200 hover:border-orange-300 hover:shadow-lg transition-all duration-300"
    >
        ...
        <div className="max-w-8xl mx-auto">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-slate-600 max-w-3xl mx-auto">
              Fractional Finance Team services provide ongoing access to specialised
              finance talent who work with your business on a defined, recurring
              basis—without being full-time employees.
            </p>
          </motion.div>

          {/* Row 1 – 3 cards */}
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            {supportAreas.slice(0, 3).map((area, index) => (
              <SupportCard key={area.title} area={area} index={index} />
            ))}
          </div>

          {/* Row 2 – 2 cards centered */}
          <div className="flex flex-col sm:flex-row justify-center gap-6 mb-12">
            {supportAreas.slice(3).map((area, index) => (
              <div key={area.title} className="w-full max-w-md">
                <SupportCard area={area} index={index + 3} />
              </div>
            ))}
          </div>
          ...
          <motion.div
            ...
            className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-8 text-center shadow-lg"
          >
            <p className="text-lg text-slate-300">
              ...
              <span className="text-white font-semibold">
                strategic and analytical finance capability
              </span>
              , not just execution.
            </p>
          </motion.div>
